...
module Util
  class DbManager
    ...
    def dump_database
      ...
      cmd="pg_dump #{ENV['AACT_STAGE_DATABASE_URL']} -v -h localhost -p 5432 -U #{ENV['AACT_DB_SUPER_USERNAME']} --no-password --clean --exclude-table schema_migrations --schema ctgov -c -C -Fc -f #{fm.pg_dump_file}"
      run_command_line(cmd)
