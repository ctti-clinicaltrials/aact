...
module Util
  class DbManager
    ...
    def terminate_db_sessions(db_name)
      public_con.execute("SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE pid <> pg_backend_pid() AND datname ='#{db_name}' AND usename <> '#{AACT_DB_SUPER_USERNAME}'")
    end
