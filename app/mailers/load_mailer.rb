class LoadMailer < ApplicationMailer
  ...
  def send_notification(email, load_event)
    subject_line="#{Rails.env.capitalize} #{load_event.event_type.capitalize} Load: Status: #{load_event.status}. Add: #{load_event.should_add} Update: #{load_event.should_change} Studies processed:  #{load_event.processed}"
