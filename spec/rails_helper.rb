...
abort("AACT_ADMIN_DATABASE_URL environment variable is not set")   if !ENV["AACT_ADMIN_DATABASE_URL"]
abort("AACT_BACK_DATABASE_URL environment variable is not set")    if !ENV["AACT_BACK_DATABASE_URL"]
abort("AACT_STAGE_DATABASE_URL environment variable is not set")   if !ENV["AACT_STAGE_DATABASE_URL"]
abort("AACT_PUBLIC_DATABASE_URL environment variable is not set")  if !ENV["AACT_PUBLIC_DATABASE_URL"]
abort("AACT_PUBLIC_HOSTNAME environment variable is not set")      if !ENV["AACT_PUBLIC_HOSTNAME"]
abort("AACT_PUBLIC_DATABASE_NAME environment variable is not set") if !ENV["AACT_PUBLIC_DATABASE_NAME"]
abort("AACT_DB_SUPER_USERNAME environment variable is not set") if !ENV["AACT_DB_SUPER_USERNAME"]
