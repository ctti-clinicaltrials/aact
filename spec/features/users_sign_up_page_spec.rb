...
feature "Users Sign Up Page" do
  ...
  scenario "sign up - verify new user validations" do
    ...
    expect(UserEventMailer).to receive(:report_user_event).exactly(UserEventMailer.admin_addresses.size).times
    submit
    ...
    expect(UserEventMailer).to receive(:report_user_event).exactly(1).times
    submit
    user=User.where('username=?',valid_username).first
