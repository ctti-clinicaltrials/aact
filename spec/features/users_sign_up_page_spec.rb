...
feature "Users Sign Up Page" do
  ...
  scenario "sign up - verify new user validations" do
    ...
    expect(UserMailer).to receive(:report_event).exactly(ApplicationMailer.admin_addresses.size).times
    submit
    ...
    expect(UserMailer).to receive(:report_event).exactly(1).times
    submit
    user=User.where('username=?',valid_username).first
