...
RSpec.describe DataDefinition, type: :model do
  ...
  it "populates for each table and saves correct row counts" do
    ...
    expect(DataDefinition.where('table_name=? and column_name=?','outcome_measurements','id').first.row_count).to eq(study.outcome_measurements.size)
    expect(DataDefinition.where('table_name=? and column_name=?','browse_conditions','id').first.row_count).to eq(study.browse_conditions.size)
    expect(DataDefinition.where('table_name=? and column_name=?','facilities','id').first.row_count).to eq(study.facilities.size)
    expect(DataDefinition.where('table_name=? and column_name=?','outcome_counts','id').first.row_count).to eq(study.outcome_counts.size)
    expect(DataDefinition.where('table_name=? and column_name=?','baseline_counts','id').first.row_count).to eq(study.baseline_counts.size)
