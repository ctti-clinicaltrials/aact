...
RSpec.describe Criterium, type: :model do
  context 'when criteria provided' do
    ...
    it "should handle all uppercase 'INCLUSION/EXLUSION CRITERIA' labels' '" do
      ...
      expect(inclusion.size).to eq(13)
      expect(exclusion.size).to eq(2)
    end

    it 'should handle mixed-case inc/excl labels' do
      described_class.destroy_all
      nct_id='NCT00151216'
      xml=Nokogiri::XML(File.read("spec/support/xml_data/#{nct_id}.xml"))
      opts={xml: xml, nct_id: nct_id}
